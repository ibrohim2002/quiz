...
const filteredLessons = computed(() =>
	props.lessons.filter(
		l =>
			l.comingSoon ||
			(l.exercises?.length ?? 0) > 0,
	),
);

function lessonSummary(lesson: Lesson): string {
	if (!lesson.exercises?.length) return '';
	const total = lesson.exercises.reduce((s, sec) => s + sec.items.length, 0);
	return `${lesson.exercises.length} bo'lim · ${total} ta mashq`;
}
</script>
...
<template>
	<div>
		...
		>
			...
			>
				Hozircha mashq qo'shilgan dars yo'q
			</div>
